...
if(isset($_POST['submit']))
{
    ...
    if(isset($_POST['name']) && !empty($_POST['name']) AND isset($_POST['email']) && !empty($_POST['email']))
    {
      ...
      // {
        ...
        if($output_rows['u_name'] == $name && $output_rows['adm_no'] == $adm_no) //compare local var with db
        {
          ...
          if($output['adm_no'] != $adm_no && $output['active'] != 1)//compare if account already exists
          {
            ...
            https://'.$localIP.'/aMoc/functions/register/verify.php?email='.$email.'&hash='.$hash.'
            ...
            if($mailsend==1)
            {
              ...
              alert(message); window.location.href = "../../index.php";
              </script>';
              <?php
              //echo $msg ;
            }
          ...
          else
          {
            ...
            ?>
              <script> alert('You have already created an account, Please login using account details.'); window.location.href = "../../index.php";</script>';
            <?php
      ...
      ?>
          <script> alert('You are not eligible to make an account, Please register using valid details.'); window.location.href = "../../user_signup.php";</script>';
       <?php
  ...
  {
    ?>
    <script> alert('passwords do not match'); window.location.href = "../../user_signup.php";</script>';
    <?php
  }
}
